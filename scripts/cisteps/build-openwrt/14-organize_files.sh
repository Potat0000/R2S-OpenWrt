...
mv "${HOST_BIN_DIR}/.config" "${HOST_BIN_DIR}/config.seed"
...
if [ "x${OPT_PACKAGE_ONLY}" != "x1" ]; then
  ...
  cp "${HOST_BIN_DIR}/config.seed" "${HOST_WORK_DIR}/openwrt_firmware/"
  ...
  zip -r "${HOST_WORK_DIR}/BUILDNAME-${BUILD_TARGET}-BUILDTIME-Firmware.zip" *
fi

cd "${HOST_BIN_DIR}"
zip -r "${HOST_WORK_DIR}/BUILDNAME-${BUILD_TARGET}-BUILDTIME-Package" *
